-- 109_v_gps_task_analysis_id.sql
-- LADRILLO 2+3 — expose the period report's PK as `id` so the resolver can paginate the
-- view deterministically (cmFetchAll orders by id; the view had no unique column before).
-- Additive: same definition as migration 108 + `r.id` as the first column.
...
select
  r.id,                                                       -- unique key for pagination
  r.club_id,
  r.session_id,
  ts.session_date,
  ts.team_id,
