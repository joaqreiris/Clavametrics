-- ============================================================================
-- 052_programme_phases_fix.sql
-- La 051 se corrió en la DB con un esquema previo (description/start_week/
-- duration_weeks/display_order, sin plan_id). El editor del Rehab Planner
-- consulta otro esquema (plan_id, week_start, week_end, objective, load_level,
-- phase_order) y rompe con "Could not find the 'objective' column...".
--
-- Como programme_phases todavía no tiene datos reales, dropeamos y recreamos
-- con el esquema correcto. Idempotente.
--
-- CÓMO CORRER: en el SQL Editor, primero DEV. Validá, y después PROD.
-- ============================================================================

drop table if exists public.programme_phases cascade;
...
create table if not exists public.programme_phases (
  ...
  load_level  text,                                -- carga (low/medium/high…) (tooltip)
