-- 160 — El pipeline suma el idioma del contacto y la baja de los mails.
--
-- Dos columnas nuevas sobre la 155: `contact_lang` (en qué idioma usa la app la
-- persona, para escribirle en el suyo) y `emails_opt_out` (si pidió que no le
-- escribamos). Las consume la Edge Function trial-emails.
--
-- Reemplaza a la 155 entera: con OUT params que cambian no alcanza un replace.
--
...
as $$
  with contacto as (
    select distinct on (p.club_id)
           ...
           p.email, p.phone, p.job_title, p.preferred_lang, p.marketing_opt_in
  ...
  select c.id,
         ...
         k.nombre, k.email, k.phone, k.job_title, k.preferred_lang,
         coalesce(k.marketing_opt_in, false),
         coalesce(c.trial_emails_opt_out, false),
...
comment on function public.sales_pipeline_rows() is
  'El cálculo del pipeline comercial, sin control de acceso. Solo service_role: las llaman las Edge Functions hubspot-sync y trial-emails. Para la app está sales_pipeline(), que es esta con el guard de platform admin.';
