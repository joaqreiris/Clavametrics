      ...
      if (window.assessNorms){
        ...
        if (rs.length){
          ratiosHtml = `<div class="pp-as-ratios"><span class="pp-as-sec">${esc(T('player.derived_ratios','Derived ratios'))}</span>`
            + rs.map(r => `<span class="pp-as-ratio" title="${esc(r.reference||'')}">${esc(T('evaluations.ratio_'+r.key, r.key))}${r.angle?' '+esc(r.angle):''}${r.side&&r.side!=='NA'?' '+esc(r.side):''} <span class="rv">${esc((Math.round(r.value*100)/100).toFixed(2))}</span></span>`).join('')
            + `</div>`;
        }
      }
