...
`;window.SettingsDrawer=Q;window.openCMSettings=null;window.initCMSettings=P;function Z(a){let o=document.querySelector('[title="Notifications"],[aria-label="Notifications"]');if(!o)return;let t=o.querySelector(".cm-bell-badge");if(a<=0){t&&t.remove();return}t||(t=document.createElement("span"),t.className="cm-bell-badge",Object.assign(t.style,{position:"absolute",top:"3px",right:"3px",width:"7px",height:"7px",borderRadius:"50%",background:"var(--cm-danger,#DC2626)",pointerEvents:"none"}),o.style.position="relative",o.appendChild(t))}function ve(){let[a,o]=React.useState(!1),[t,u]=React.useState(null),[m,g]=React.useState(null),[N,i]=React.useState(null),[_,x]=React.useState([]),D=React.useRef(null),S=React.useRef(null);React.useEffect(()=>{if(window.sb)return window.sb.auth.getSession().then(({data:p})=>{var v,w;let f=(w=(v=p==null?void 0:p.session)==null?void 0:v.user)==null?void 0:w.id;f&&(g(f),window.CM_I18N&&window.CM_I18N.setCloudSaver(async l=>{let{data:h}=await window.sb.from("profiles").select("settings").eq("id",f).single(),y={...(h==null?void 0:h.settings)||{},language:l};await window.sb.from("profiles").update({settings:y}).eq("id",f)}),window.getClub&&window.getClub().then(l=>{l&&window.CM_I18N&&window.CM_I18N.setClubCountry(l.country)}),window.sb.from("profiles").select("settings, notification_settings").eq("id",f).single().then(({data:l})=>{if(!l)return;window.CM_I18N&&l.settings&&l.settings.language&&window.CM_I18N.setUserPref(l.settings.language);let h={...l.settings||{},notif:l.notification_settings||{}};(Object.keys(l.settings||{}).length>0||Object.keys(l.notification_settings||{}).length>0)&&i(h)}),window.sb.from("notifications").select("id",{count:"exact",head:!0}).eq("user_id",f).eq("read",!1).then(({count:l})=>{l>0&&Z(l)}),S.current=window.sb.channel("cm-notif-"+f).on("postgres_changes",{event:"INSERT",schema:"public",table:"notifications",filter:`user_id=eq.${f}`},l=>{let h=l.new,y=Date.now()+Math.random();x(j=>[...j,{...h,_popupId:y}]);let k=document.querySelector('[title="Notifications"],[aria-label="Notifications"]'),A=k?parseInt(k.dataset.unread||"0",10):0;k&&(k.dataset.unread=A+1),Z(A+1)}).subscribe())}),()=>{S.current&&window.sb.removeChannel(S.current)}},[]),React.useEffect(()=>(window.openCMSettings=()=>o(!0),()=>{window.openCMSettings=null}),[]),React.useEffect(()=>{!a||t||window.getProfile&&window.getProfile().then(p=>u(p))},[a]);function E(p){!m||!window.sb||(clearTimeout(D.current),D.current=setTimeout(async()=>{let{notif:f,...v}=p,w;try{w=localStorage.getItem("cm_lang")}catch{}window.CM_I18N&&w?v.language=window.CM_I18N.current:delete v.language;let{error:l}=await window.sb.from("profiles").update({settings:v,notification_settings:f||{}}).eq("id",m);l&&console.warn("[settings] cloud save failed:",l.message)},800))}function r(p){x(f=>f.filter(v=>v._popupId!==p))}return React.createElement(React.Fragment,null,React.createElement(Q,{open:a,onClose:()=>o(!1),profile:t,userId:m,setProfile:u,supabaseSettings:N,onSettingsChange:E}),React.createElement("div",{className:"cm-notif-stack"},_.map(p=>React.createElement(ge,{key:p._popupId,notif:p,onDismiss:()=>r(p._popupId)}))))}(function(){P();let o=document.getElementById("settings-host")||(()=>{let t=document.createElement("div");return t.id="settings-host",document.body.appendChild(t),t})();ReactDOM.createRoot(o).render(React.createElement(ve,null)),document.addEventListener("click",t=>{t.target.closest("[data-open-settings]")&&(t.preventDefault(),window.openCMSettings&&window.openCMSettings())})})();})();
