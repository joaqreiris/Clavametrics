...
export function aggregateSeries(rows, eavMap, config, catalog) {
  ...
  function dimGroup(row, dimId) {
    ...
    switch (dimId) {
      ...
      // display = nombre del MC; el filtro `microcycle` matchea por training_sessions.microcycle_id
      case 'microcycle':   v = attrs.microcycle || row.training_sessions?.microcycle_id;
                           fid = row.training_sessions?.microcycle_id ?? null; break;
