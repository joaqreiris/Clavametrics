...
export async function getMcSessionIds(refMcId, ctx, sb) {
  // Both ids must be real uuids — a null / "null" / invalid refMcId yields no
  // reference (caller degrades to no comparison), never a 400 from the uuid cast.
  if (!isUuid(refMcId) || !isUuid(ctx.clubId)) return [];
...
export async function resolveCard(config, ctx, catalog, sb) {
  try {
    ...
    if (config.comparison?.baseline === 'mc' && isUuid(config.comparison?.refMcId)) {
      try {
