...
async function _nonTeamPeriodSums(sb, clubId, sessionIds, playerIds) {
  if (!sessionIds || !sessionIds.length) return new Map();
  if (typeof window === 'undefined' || !window.cmFetchAll) return new Map();
  const sel = ['session_id','player_id', ..._ADDITIVE_COLS].join(',');
  ...
  try {
    if (typeof window !== 'undefined' && window.cmFetchAll) {
      ...
      if (!isTask && rows.length) {
        const sums = await _nonTeamPeriodSums(sb, ctx.clubId, sessionIds, _pidScoped ? [_pid] : (ctx.teamPlayerIds || null));
