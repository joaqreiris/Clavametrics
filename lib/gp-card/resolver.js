...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  if (config.scope.level === 'player') {
    let pid = config.scope.playerId || ctx.playerId;
    // Si el jugador elegido no pertenece al equipo activo, usar el primero del equipo
    // (evita "No data" por el scope de equipo y nunca muestra jugadores de otra categoría).
    if (Array.isArray(ctx.teamPlayerIds) && ctx.teamPlayerIds.length && pid && !ctx.teamPlayerIds.includes(pid)) {
      pid = ctx.teamPlayerIds[0];
    }
    if (!isUuid(pid)) return [];          // null/"null"/invalid → no rows, never a uuid 400
    q = q.eq('player_id', pid);
  } else if (Array.isArray(ctx.teamPlayerIds) && ctx.teamPlayerIds.length) {
    // Squad scope: limitar a jugadores del equipo activo.
