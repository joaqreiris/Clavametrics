 ...
 */
// ── Modelo A (2026-08): trabajo no-team = SOLO VOLUMEN ────────────────────────────
// El trabajo no-team (rehab/individual/top-up) SUMA en los totales de cada jugador (volumen
// completo, siempre visible) pero se EXCLUYE de todas las MEDIAS cross-player (media del día,
// línea AVG, rollup de plantel). Un jugador que hizo trabajo no-team en una sesión queda fuera
// de la media de esa sesión ENTERO (no se le resta un pedazo). La media del equipo es SIEMPRE
// limpia — el filtro Context sirve para aislar/ver, no para ensuciar el promedio.
//
// Antes se RESTABA el volumen no-team del total de sesión (Fase 2b): eso limpiaba la media pero
// corrompía el total. Ahora no se resta: se MARCAN las filas (session_id, player_id) con período
// no-team para que aggregateSeries las excluya solo de las medias.
async function _nonTeamMeanKeys(sb, clubId, sessionIds, playerIds) {
  const keys = new Set();
  if (!sessionIds || !sessionIds.length) return keys;
  if (typeof window === 'undefined' || !window.cmFetchAll) return keys;
  const rows = await window.cmFetchAll(() => {
    ...
    return q;
  }, { label: 'resolver.nonTeamMeanKeys' });
  for (const r of rows) keys.add(r.session_id + '|' + r.player_id);
  return keys;
}
...
async function _ctxPeriodRows(sb, ctx, sessionIds, contexts, playerIds, existingRows, select) {
  ...
  const agg = new Map();   // 'sid|pid' → sumas de los períodos del contexto
  ...
  for (const r of base) {
    const a = agg.get(r.session_id + '|' + r.player_id);
...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  try {
    if (typeof window !== 'undefined' && window.cmFetchAll) {
      ...
      const _wcNT = _wc.filter(c => c !== 'team');
      if (!isTask && _wcNT.length) {
        ...
        const extra = await _ctxPeriodRows(sb, ctx, sessionIds, _wcNT, _pidScoped ? [_pid] : _rosterLimit, rows, select);
        if (extra.length) rows.push(...extra);
      }
      // Modelo A: NO se resta el volumen (se ve completo). Se MARCAN las filas (session, player)
      // con período no-team para que aggregateSeries las excluya SOLO de las medias cross-player.
      // Si la vista es EXCLUSIVAMENTE no-team (aislamiento puro), no se marca: la media que se ve
      // es la media DEL contexto aislado (no hay media de equipo que ensuciar en esa vista).
      const _pureCtx = _wcNT.length && !_wc.includes('team');
      if (!isTask && rows.length && !_pureCtx) {
        const ntKeys = await _nonTeamMeanKeys(sb, ctx.clubId, sessionIds, _pidScoped ? [_pid] : (ctx.teamPlayerIds || null));
        if (ntKeys.size) for (const r of rows) {
          const sid = r.session_id ?? r.training_sessions?.id;
          if (sid != null && ntKeys.has(sid + '|' + r.player_id)) r._excludeMean = true;
        }
      }
      return rows;
    }
...
// Agregaciones que son MEDIAS (central-tendency): excluyen las filas marcadas _excludeMean
// (jugador con período no-team en esa sesión). Las de suma/total/máx/mín usan a todos (volumen).
