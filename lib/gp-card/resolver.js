...
// Las consultas de PERÍODO se repiten IDÉNTICAS entre cards: mismo club, mismas sesiones, mismo
// contexto. Sin compartirlas, un dashboard de diez cards pide lo mismo diez veces (en datos reales
// se vieron ~15 consultas seguidas a gps_period_reports, varias de más de un segundo). Se comparte
// la PROMESA, igual que getSessionIds/fetchReports, así que N cards pagan un solo viaje.
// SÓLO se cachea lo que el llamador LEE sin mutar: _aggPeriodRows y los pares «dirty» solo leen.
// Las filas de gps_reports de _ctxPeriodRows NO se cachean a propósito — ahí se pisan los valores
// fila por fila (_applyPeriodAgg), y compartir esos objetos cruzaría datos entre cards.
// Las claves ordenan los ids: dos cards con las mismas sesiones en distinto orden deben acertar.
const _idsKey = (a) => (a && a.length) ? [...a].sort().join(',') : '';
function _periodCached(key, hacer) {
  const hit = _cacheGet(key);
  if (hit !== undefined) return hit;
  const p = hacer().catch(e => {
    // un rechazo no se reusa: la próxima card vuelve a intentar
    if (typeof window !== 'undefined' && window.__gpResolverCache) window.__gpResolverCache.delete(key);
    throw e;
  });
  return _cacheSet(key, p);
}
...
export const DERIVED = {
  ...
  // La versión por minuto: sumar las dos columnas /min de la vista de ejercicios. Sin ella, la
  // carga mecánica era la única familia que no se podía mirar normalizada por duración.
  acc_dec_per_min: ['accelerations_per_min', 'decelerations_per_min'],
};
 ...
 */
/**
 * Fecha desde la que los datos del club son COMPARABLES (club_gps_settings.gps_valid_from).
 * Un club que cambió de metodología —umbrales fijos de velocidad a % de la Vmax, por ejemplo—
 * tiene un antes y un después que no se pueden mezclar en el mismo gráfico. Las filas viejas
 * siguen en la base; simplemente no entran al análisis.
 * Se cachea por club: es un ajuste, no un dato que cambie mientras se mira un dashboard.
 */
const _validFromCache = new Map();
async function _gpsValidFrom(clubId, sb) {
  if (!isUuid(clubId)) return null;
  if (_validFromCache.has(clubId)) return _validFromCache.get(clubId);
  const p = (async () => {
    try {
      const { data, error } = await sb.from('club_gps_settings')
        .select('gps_valid_from').eq('club_id', clubId).maybeSingle();
      if (error) throw error;
      return data?.gps_valid_from || null;
    } catch (_e) {
      // Sin la columna (migración no aplicada) el análisis sigue como siempre: sin corte.
      return null;
    }
  })();
  _validFromCache.set(clubId, p);
  return p;
}
...
export async function getSessionIds(range, ctx, sb) {
  ...
  // Corte de comparabilidad del club: lo anterior no entra a NINGUNA card. Va acá porque es el
  // único sitio por el que pasan todas — cualquier rango, cualquier tipo de gráfico.
  const _validFrom = await _gpsValidFrom(ctx.clubId, sb);
  if (_validFrom) q = q.gte('session_date', _validFrom);
 ...
 */
// ── Modelo B (2026-08): el filtro Context es DINÁMICO por período ─────────────────
// Los valores de cada (session, player) reflejan EXACTAMENTE los contextos seleccionados en el
// bar (default: solo 'team'). Un par cuyo trabajo incluye períodos de OTROS contextos se
// RECALCULA sumando solo sus períodos de los contextos pedidos (m/min = Σdist / Σmin de esos
// períodos); si NINGÚN período suyo cae en los contextos pedidos, el jugador desaparece de la
// vista. Así «Team» solo muestra trabajo de equipo (sin rehab ni top-up), «Rehab» solo muestra a
// quienes hicieron rehab, y «Team + Top-up» suma ambos — sin marcar filas ni ensuciar medias.
// Reemplaza al Modelo A (marcado _excludeMean): las filas ya llegan puras al agregador.
// ¿estas sesiones tienen ALGÚN período no-team? (para que el fast-path RPC — que no resta —
// caiga al camino crudo, que sí resta). Cheap: head-count sobre el índice parcial.
export async function hasNonTeamPeriods(sb, clubId, sessionIds) {
  if (!sessionIds || !sessionIds.length) return false;
  return _periodCached(`nonteam|${clubId}|${_idsKey(sessionIds)}`, async () => {
    try {
      const { count } = await sb.from('gps_period_reports')
        .select('id', { count: 'exact', head: true })
        .eq('club_id', clubId).neq('work_context', 'team').in('session_id', sessionIds);
      return (count || 0) > 0;
    } catch (_e) { return false; }
  });
}

// Aislar contextos etiquetados por PERÍODO. La etiqueta rehab/individual/top-up vive casi
// siempre en gps_period_reports (en clubes con API el re-sync diario borra la de sesión), así
// que filtrar solo por gps_reports.work_context devolvía 0 filas («No data») al seleccionar un
// contexto en el bar. Para cada (session, player) con períodos en los contextos pedidos y SIN
// fila ya incluida por la etiqueta de sesión, se toma su fila real de gps_reports (por el join
// de sesión/jugador) y se REEMPLAZAN sus valores por la suma de esos períodos: el aislamiento
// muestra la carga DEL CONTEXTO, no el total mixto de la sesión (verificado en datos reales:
// período INDIVIDUAL = 1.4k m dentro de una sesión de 3.9k m).
// Limitación conocida: métricas EAV (extra_metrics) siguen mostrando el valor de sesión completa.
const _CTX_SUM_COLS = ['total_distance', 'high_speed_distance', 'very_high_speed_distance',
  'sprint_distance', 'sprint_count', 'accelerations', 'decelerations', 'player_load', 'hmld', 'time_played'];
// Suma filas de gps_period_reports por (session, player) → Map 'sid|pid' → acumulador.
function _aggPeriodRows(periods) {
  const agg = new Map();
  for (const p of periods) {
    const k = p.session_id + '|' + p.player_id;
    let a = agg.get(k);
    if (!a) agg.set(k, a = { sums: {}, dur: 0, maxSpeed: null, spdW: 0, spdWd: 0 });
    for (const c of _CTX_SUM_COLS) if (p[c] != null) a.sums[c] = (a.sums[c] || 0) + Number(p[c]);
    if (p.duration_seconds != null) a.dur += Number(p.duration_seconds);
    if (p.max_speed != null) a.maxSpeed = Math.max(a.maxSpeed ?? -Infinity, Number(p.max_speed));
    if (p.avg_speed != null) { const w = Number(p.duration_seconds) || 1; a.spdW += Number(p.avg_speed) * w; a.spdWd += w; }
  }
  return agg;
}
// Pisa los valores core de una fila de gps_reports con el acumulador de períodos. m/min y
// time_played se derivan de la duración REAL de esos períodos (time_played si vino, si no
// duration_seconds/60) — nunca del total de sesión.
function _applyPeriodAgg(r, a) {
  for (const c of _CTX_SUM_COLS) r[c] = (c in a.sums) ? a.sums[c] : null;
  r.max_speed = a.maxSpeed;
  r.avg_speed = a.spdWd > 0 ? a.spdW / a.spdWd : null;
  const mins = (a.sums.time_played > 0) ? a.sums.time_played : (a.dur / 60);
  r.time_played = mins || null;
  r.distance_per_minute = (mins > 0 && a.sums.total_distance != null) ? a.sums.total_distance / mins : null;
}
async function _ctxPeriodRows(sb, ctx, sessionIds, contexts, playerIds, existingRows, select) {
  if (typeof window === 'undefined' || !window.cmFetchAll) return [];
  const periods = await _periodCached(
    `ctxperiods|${ctx.clubId}|${_idsKey(sessionIds)}|${contexts.join(',')}|${_idsKey(playerIds)}`,
    () => window.cmFetchAll(() => {
      let q = sb.from('gps_period_reports')
        .select('session_id,player_id,duration_seconds,avg_speed,max_speed,' + _CTX_SUM_COLS.join(','))
        .eq('club_id', ctx.clubId).in('work_context', contexts).in('session_id', sessionIds);
      if (playerIds && playerIds.length) q = q.in('player_id', playerIds);
      return q;
    }, { label: 'resolver.ctxPeriods' }));
  if (!periods.length) return [];
  const agg = _aggPeriodRows(periods);   // 'sid|pid' → sumas de los períodos del contexto
  // Pares ya presentes (etiqueta de SESIÓN en los contextos, o 'team' también seleccionado —
  // ahí la fila muestra el total completo, Modelo A): no duplicar ni pisar.
  const have = new Set(existingRows.map(r => r.session_id + '|' + r.player_id));
  const want = new Set([...agg.keys()].filter(k => !have.has(k)));
  if (!want.size) return [];
  const sids = [...new Set([...want].map(k => k.split('|')[0]))];
  const pids = [...new Set([...want].map(k => k.split('|')[1]))];
  let base = await window.cmFetchAll(() => sb.from('gps_reports').select(select)
    .eq('club_id', ctx.clubId).eq('is_invalid', false)
    .in('session_id', sids).in('player_id', pids), { label: 'resolver.ctxPeriodBase' });
  const archived = new Set(ctx.archivedPlayerIds || []);
  base = base.filter(r => want.has(r.session_id + '|' + r.player_id) && !archived.has(r.player_id));
  for (const r of base) {
    _applyPeriodAgg(r, agg.get(r.session_id + '|' + r.player_id));
    r._ctxPeriod = true;
  }
  return base;
}

// Modelo B — recorte dinámico por contexto. Detecta los pares (session, player) con períodos
// FUERA de los contextos pedidos («dirty»: su fila de sesión trae trabajo ajeno mezclado) y les
// recalcula los valores sumando SOLO sus períodos de los contextos pedidos; sin ningún período
// en esos contextos, la fila se elimina (el jugador no participó de lo que se está mirando).
// Períodos con work_context NULL (data legacy sin etiquetar) cuentan como 'team'.
// Los pares sin períodos ajenos quedan intactos (camino común: cero costo extra tras 1 query).
// Limitación conocida: métricas EAV (extra_metrics) siguen mostrando el valor de sesión completa.
async function _applyCtxScope(sb, ctx, sessionIds, contexts, playerIds, rows) {
  if (typeof window === 'undefined' || !window.cmFetchAll) return rows;
  const dirty = await _periodCached(
    `ctxdirty|${ctx.clubId}|${_idsKey(sessionIds)}|${contexts.join(',')}|${_idsKey(playerIds)}`,
    () => window.cmFetchAll(() => {
      let q = sb.from('gps_period_reports').select('session_id,player_id')
        .eq('club_id', ctx.clubId).in('session_id', sessionIds)
        .not('work_context', 'in', `(${contexts.join(',')})`);
      if (playerIds && playerIds.length) q = q.in('player_id', playerIds);
      return q;
    }, { label: 'resolver.ctxDirtyPairs' }));
  if (!dirty.length) return rows;
  const dirtyKeys = new Set(dirty.map(p => p.session_id + '|' + p.player_id));
  const affected = rows.filter(r => dirtyKeys.has(r.session_id + '|' + r.player_id));
  if (!affected.length) return rows;
  const sids = [...new Set(affected.map(r => r.session_id))];
  const pids = [...new Set(affected.map(r => r.player_id))];
  const withTeam = contexts.includes('team');
  const periods = await _periodCached(
    `ctxscope|${ctx.clubId}|${_idsKey(sids)}|${_idsKey(pids)}|${contexts.join(',')}|${withTeam}`,
    () => window.cmFetchAll(() => {
      let q = sb.from('gps_period_reports')
        .select('session_id,player_id,duration_seconds,avg_speed,max_speed,' + _CTX_SUM_COLS.join(','))
        .eq('club_id', ctx.clubId).in('session_id', sids).in('player_id', pids);
      return withTeam
        ? q.or(`work_context.in.(${contexts.join(',')}),work_context.is.null`)
        : q.in('work_context', contexts);
    }, { label: 'resolver.ctxScopePeriods' }));
  const agg = _aggPeriodRows(periods);
  return rows.filter(r => {
    const k = r.session_id + '|' + r.player_id;
    if (!dirtyKeys.has(k)) return true;
    const a = agg.get(k);
    if (!a) return false;
    _applyPeriodAgg(r, a);
    r._ctxAdj = true;
...
// Orquestador del Modelo B, EXPORTADO para las vistas con fetch propio (mc-compare,
// session-control, match-perf): síntesis de filas para contextos no-team (si se pasa `select`,
// con la MISMA forma de columnas del caller) + recorte dinámico. `ctx` solo necesita clubId
// (opcionalmente archivedPlayerIds). Muta/filtra `rows` y devuelve el array resultante.
export async function applyCtxToRows(sb, ctx, sessionIds, workContexts, playerIds, rows, select) {
  const S = (workContexts && workContexts.length) ? workContexts : ['team'];
  const nt = S.filter(c => c !== 'team');
  if (nt.length && select) {
    const extra = await _ctxPeriodRows(sb, ctx, sessionIds, nt, playerIds, rows, select);
    if (extra.length) rows.push(...extra);
  }
  if (!rows.length) return rows;
  return _applyCtxScope(sb, ctx, sessionIds, S, playerIds, rows);
}
...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  // Training context (rehab / individual / top-up): by DEFAULT only 'team' work counts, so
  // rehab/individual/top-up rows never skew a squad or player mean. The filter bar's "Context"
  // dimension widens this (getState().workContexts) to view those contexts in isolation.
  let _wc = (typeof window !== 'undefined' && window.gpFilterBar?.getState?.().workContexts) || [];
  // Una card que AGRUPA por contexto los quiere ver todos: pedir «separá por contexto» y recibir
  // una sola barra («Equipo», porque el default trae sólo eso) no es lo que nadie quiso decir.
  // Si el usuario acotó los contextos en la barra, manda su elección.
  if (!_wc.length && (config.dimensions || []).some(d => d.id === 'work_context')) {
    _wc = ['team', 'rehab', 'individual', 'topup'];
  }
  const _wcKey = _wc.length ? _wc.slice().sort().join('|') : 'team';
  ...
  const _ck = `rpts:${config.source||'session'}:${ctx.clubId}:${ctx.teamId||''}:${_pid}:${_wcKey}:${(ctx.teamPlayerIds||[]).length}:${(ctx.archivedPlayerIds||[]).length}:${sessionIds.slice().sort().join(',')}`;
  const _hit = _cacheGet(_ck);
  if (_hit !== undefined) return _hit;
  return _cacheSet(_ck, (async () => {
  ...
  const select = isTask ? '*' : [
    ...
    // session_type: lo usa el radar para avisar cuando una comparación mezcla partidos con
    // entrenamientos (volúmenes muy distintos → el promedio no mide lo mismo para cada jugador).
    'training_sessions!inner(session_date, session_attributes, microcycle_id, match_day_offset, session_type)',
    // always fetch player name
    'players!inner(id, first_name, last_name, position)',
    // contexto del trabajo (team / rehab / individual / top-up): la fila ya se filtra por él, y
    // desde que se puede AGRUPAR por contexto también hace falta leerlo.
    'work_context',
    ...coreNeeded,
  ].join(',');
  ...
  const buildQ = () => {
    ...
    if (!isTask) {
      q = q.eq('is_invalid', false);   // the view already excludes flagged periods
      // Training context filter (see _wc above): default 'team' only; widened by the bar.
      q = _wc.length ? q.in('work_context', _wc) : q.eq('work_context', 'team');
    }
    if (_pidScoped) {
  ...
  try {
    if (typeof window !== 'undefined' && window.cmFetchAll) {
      const rows = await window.cmFetchAll(buildQ, { label: 'resolver.fetchReports' });
      if (isTask) return rows;
      // Modelo B: síntesis para contextos no-team (la etiqueta suele vivir en PERÍODOS, no en la
      // fila de sesión) + recorte dinámico a los contextos pedidos. Ver applyCtxToRows.
      const _rosterLimit = (!_pidScoped && !isUuid(ctx.teamId) && Array.isArray(ctx.teamPlayerIds) && ctx.teamPlayerIds.length)
        ? ctx.teamPlayerIds : null;
      return await applyCtxToRows(sb, ctx, sessionIds, _wc, _pidScoped ? [_pid] : _rosterLimit, rows, select);
    }
...
// Agregaciones que son MEDIAS (central-tendency): excluían las filas marcadas _excludeMean
// (Modelo A). Con el Modelo B (recorte dinámico en fetchReports) el flag ya no se setea — las
// filas llegan puras — pero los checks quedan como no-op inofensivo por compatibilidad.
// MD "crudo" de una sesión: la COLUMNA match_day_offset primero (lo que escribe Daily
// Planning — con dos dinámicas el mismo día cada sesión tiene el suyo), luego
// session_attributes.md_code. Mismo orden y mismo formato que _mdRaw del filter bar y
// _gpMdOf de gp-builder; string tal cual (SIN normalizar) para que agrupar == opción del
// filtro == narrowing resuelvan el mismo valor.
function mdRawOf(ts) {
  const off = ts ? ts.match_day_offset : null;
  if (off != null && off !== '') {
    if (typeof off === 'string') return off;
    return off === 0 ? 'MD' : (off < 0 ? `MD${off}` : `MD+${off}`);
  }
  return (ts && ts.session_attributes && ts.session_attributes.md_code) || '';
}

const _MEAN_AGGS = new Set(['avg', 'wavg', 'median']);
...
export function aggregateSeries(rows, eavMap, config, catalog) {
  ...
  function timeLabel(row) {
    ...
    // La fecha, en las DOS formas: anidada (join a training_sessions) o plana, que es como la
    // trae la vista de ejercicios. Leyendo sólo la anidada, una línea de tareas metía todo en un
    // único punto llamado «?».
    const _sd = row.training_sessions?.session_date ?? row.session_date;
    return _sd ? String(_sd).slice(0, 10) : '?';
  }
  ...
  /** Etiqueta traducida de un valor catalogado del ejercicio; el valor crudo si no hay. */
  function _taskLabel(kind, raw) {
    if (raw == null || raw === '') return null;
    const fn = (typeof window !== 'undefined') ? window.gpTaskLabel : null;
    return (fn && fn(kind, raw)) || String(raw);
  }
  function dimGroup(row, dimId) {
    ...
    switch (dimId) {
      ...
      // MD: el de la SESIÓN primero (match_day_offset de Daily Planning → md_code), vía mdRawOf —
      // con dos dinámicas el mismo día cada sesión agrupa por su propio MD. Si la sesión no tiene
      // MD propio, el MD por fecha del filter bar (∪ DERIVADO del microciclo = partido +
      // md_overrides). Así las sesiones de GPS importadas (sin MD propio) muestran su MD.
      case 'md_code': {
        const _sd = row.training_sessions?.session_date ?? row.session_date;
        const _dd = _sd ? String(_sd).slice(0, 10) : '';
        v = mdRawOf(row.training_sessions)
      ...
      // Contexto del trabajo. NULL = dato viejo sin etiquetar, que en todo el producto cuenta
      // como 'team' (misma regla que _applyCtxScope), así que acá también.
      case 'work_context': { const _wc = row.work_context || 'team';
                             v = (typeof window !== 'undefined' && window.gpWorkContextLabel)
                                 ? window.gpWorkContextLabel(_wc) : _wc;
                             fid = _wc; break; }
      ...
      // Catálogo de la biblioteca. Se muestran en el idioma del usuario cuando hay traducción
      // (ENDURANCE → «Resistencia»); sin ella, el valor tal cual, que es lo que el club escribió.
      case 'orientation':       v = _taskLabel('orientation', row.orientation); break;
      case 'planned_intensity': v = _taskLabel('intensity', row.planned_intensity); break;
  ...
  const GROUPED = ['bars', 'ranking', 'table', 'heatmap', 'scatter', 'line', 'box'].includes(config.viz);
  ...
  // EXCEPCIÓN 2 — dimensión de EJERCICIO (drill · tamaño de campo · formato): el punto es el
  // EJERCICIO, no el jugador. Un jugador hace varios drills en la misma sesión, así que dejarlo
  // como entidad daba un punto por jugador con un color sacado del primer drill que apareciera:
  // ni comparaba ejercicios ni coloreaba nada. Con esto, «HMLD vs intensidad» sobre tareas
  // compara los ejercicios entre sí, que es para lo que se abre.
  const TASK_DIMS = new Set(['drill', 'field_size', 'players_format']);
  const scatterByTask = config.viz === 'scatter' && !scatterMatchDemand && TASK_DIMS.has(dimIds[0]);
  const groupOf = scatterMatchDemand
    ...
    : scatterByTask
    ? (row => { const g = dimGroup(row, dimIds[0]);
                return { key: g.key, label: g.label, dims: [g.label], cat: g.label, fid: g.fid }; })
    : config.viz === 'scatter'
  ...
  return config.metrics.map(m => {
    ...
    const isMean = _MEAN_AGGS.has(m.agg);
    if (GROUPED) {
      ...
      for (const row of rows) {
        const g = groupOf(row);
        if (!byKey.has(g.key)) byKey.set(g.key, { label: g.label, dims: g.dims, cat: g.cat, pid: g.pid, fid: g.fid, vals: [], weights: [], sids: isCount ? new Set() : null, nonTeam: false });
        const e = byKey.get(g.key);
        if (row._excludeMean) e.nonTeam = true;   // el grupo tuvo trabajo no-team → la línea AVG lo excluye
        if (isCount) { const sid = sessOf(row); if (sid != null) e.sids.add(sid); continue; }
        const v = valOf(row);
        // Media (avg/wavg/median): la fila no-team NO cuenta. Suma/total/máx/mín: cuenta (volumen).
        if (v != null && !isNaN(v) && !(isMean && row._excludeMean)) { e.vals.push(v); e.weights.push(rowWeight(row)); }
      }
      const points = Array.from(byKey.values())
        .map(({ label, dims, cat, pid, fid, vals, weights, sids, nonTeam }) =>
          ({ x: label, dims, cat, pid, fid, _nonTeam: nonTeam, y: isCount ? sids.size : (applyAgg(vals, m.agg, weights) ?? 0) }));
    ...
    if (rollup && rollup !== 'pooled') {
      const rollupIsMean = _MEAN_AGGS.has(rollup);
      const byPlayer = new Map();
      for (const row of rows) {
        const v = valOf(row);
        if (v == null || isNaN(v)) continue;
        const pid = row.player_id ?? playerLabel(row);
        if (!byPlayer.has(pid)) byPlayer.set(pid, { vals: [], weights: [], nonTeam: false });
        const e = byPlayer.get(pid);
        if (row._excludeMean) e.nonTeam = true;
        if (isMean && row._excludeMean) continue;   // level-1 media: la fila no-team no cuenta
        e.vals.push(v); e.weights.push(rowWeight(row));
      }
      const perPlayer = [];
      for (const { vals, weights, nonTeam } of byPlayer.values()) {
        if (rollupIsMean && nonTeam) continue;       // level-2 media limpia: el jugador no-team queda fuera entero
    ...
    const vals = [], weights = [];
    // Media (single-value, pooled): excluye filas no-team. Suma/total: usa a todos (volumen).
    for (const row of rows) { const v = valOf(row); if (v != null && !isNaN(v) && !(isMean && row._excludeMean)) { vals.push(v); weights.push(rowWeight(row)); } }
...
export async function resolvePlayerAggSeries(sessionIds, config, ctx, catalog, sb) {
  ...
  // Fase 2b: el RPC NO resta períodos no-team → si estas sesiones tienen alguno, caemos al
  // camino crudo (fetchReports), que sí resta. Sin períodos etiquetados esto es un head-count barato.
  if (await hasNonTeamPeriods(sb, ctx.clubId, sessionIds)) return null;
...
export async function resolvePlayerMcAggSeries(sessionIds, config, ctx, catalog, sb) {
  ...
  // Fase 2b: idem — con períodos no-team en scope, caer al crudo (que resta). Barato si no hay.
  if (await hasNonTeamPeriods(sb, ctx.clubId, sessionIds)) return null;
 ...
 */
// Mínimo de compañeros para que una media de puesto signifique algo. Por debajo, se sube de
// nivel; si ni al nivel más amplio se llega, igual se usa lo que haya (y la UI muestra el n).
const ROLE_MIN_PEERS = 3;
const _POS_LEVELS = ['detailed', 'basic', 'group'];
function _posAtLevel(raw, level) {
  try {
    if (typeof window === 'undefined' || !window.cmPositionAt) return raw;
    return window.cmPositionAt(raw, level);
  } catch (_e) { return raw; }
}
// El puesto de una fila. Las de sesión lo traen anidado (join a players); las de EJERCICIOS
// (v_gps_task_analysis) lo traen plano, y leyendo sólo el anidado el grupo de puesto salía
// vacío: una card de drills comparada «vs puesto» se quedaba sin referencia.
const _rowPos = r => (r.players?.position ?? r.position ?? null);

// Devuelve { rows, peers, level }: las filas de los COMPAÑEROS (nunca las del propio jugador),
// cuántos jugadores distintos son, y a qué nivel de puesto se los agrupó.
export function _roleGroupFor(rows, pid, myRaw) {
  let best = { rows: [], peers: 0, level: null };
  for (const level of _POS_LEVELS) {
    const mine = _posAtLevel(myRaw, level);
    if (mine == null) continue;
    const sel = rows.filter(r => r.player_id !== pid && _posAtLevel(_rowPos(r), level) === mine);
    const peers = new Set(sel.map(r => r.player_id)).size;
    if (peers > best.peers) best = { rows: sel, peers, level };
    if (peers >= ROLE_MIN_PEERS) break;
  }
  return best;
}

// Peso de una fila para las medias ponderadas (mismo criterio que rowWeight de aggregateSeries:
// task → segundos del período; sesión → minutos jugados; la unidad se cancela en la wavg).
function _roleRowWeight(row) {
  const d = row.duration_seconds;
  if (d != null && !isNaN(d)) return Number(d);
  const tp = row.time_played;
  return (tp != null && !isNaN(tp)) ? Number(tp) : 0;
...
// Normaliza por EXPOSICIÓN: un acumulado no es comparable entre jugadores con distinta cantidad
// de sesiones (8 partidos contra la media de 12 da un déficit del 33% aunque el rendimiento por
// partido sea idéntico — mezcla cuánto jugó con cómo rindió). Los volúmenes ('total') pasan a
// media POR SESIÓN en AMBOS lados; 'avg'/'max'/etc. ya están en escala comparable. El lado del
// jugador aplica la misma conversión en gp-builder (bloque del baseline de rol).
export const roleNormAgg = a => (a === 'total' ? 'avg' : a);
...
export async function fetchRoleBaseline(sessionIds, config, ctx, catalog, sb) {
  ...
  // Grupo de referencia = COMPAÑEROS de puesto, sin el propio jugador y con muestra suficiente.
  // Antes: (a) el jugador entraba en su propia media → el ratio se comprimía hacia 100%, y con
  // un puesto de un solo jugador daba 100% SIEMPRE; (b) si su posición era desconocida el grupo
  // pasaba a ser el plantel ENTERO, arqueros incluidos, en silencio. Ahora se sube de nivel
  // (detailed → basic 6 → grupo) hasta juntar MIN_PEERS compañeros, que es justo el roll-up que
  // positions.js documenta y que nunca se había conectado acá. Sin posición conocida o sin
  // compañeros, se devuelve vacío: el radar dibuja el valor crudo en vez de una referencia falsa.
  const myRaw = _rowPos(rows.find(r => r.player_id === pid) || {});
  const { rows: group, peers, level } = _roleGroupFor(rows, pid, myRaw);
  ...
  // Medida de tendencia central del grupo. MEDIANA por defecto: nunca es peor que la media
  // (en un grupo simétrico coinciden, y con 2 compañeros son idénticas) y es mucho mejor cuando
  // un jugador se despega — medido en este plantel, los mediocampistas dan media 389 contra
  // mediana 320 (22% de diferencia) por un solo jugador de 677 m.
  const stat = config.comparison?.stat === 'avg' ? 'avg' : 'median';

  const out = new Map();
  const ranges = new Map();
  const sds = new Map();
  for (const m of config.metrics) {
    // per-player aggregate, then central stat across the group (calc metric → per-row formula)
    const formula = isCalculated(m, catalog) ? formulaOf(m, catalog) : null;
    ...
    for (const row of group) {
      const v = cell(row);
      if (v != null && !isNaN(v)) {
        if (!byPlayer.has(row.player_id)) byPlayer.set(row.player_id, { vals: [], weights: [] });
        const b = byPlayer.get(row.player_id);
        b.vals.push(v);
        b.weights.push(_roleRowWeight(row));   // faltaban: con agg 'wavg' el baseline quedaba sin ponderar
      }
    }
    const perPlayer = Array.from(byPlayer.values())
      .map(b => applyAgg(b.vals, roleNormAgg(m.agg), b.weights))
      .filter(v => v != null && !isNaN(v));
    out.set(m.id, perPlayer.length ? applyAgg(perPlayer, stat) : null);
    // Dispersión del grupo: sin esto un 90% contra un grupo apretado y otro contra un grupo que
    // va del 50% al 200% se ven idénticos en la card, y no significan lo mismo.
    if (perPlayer.length) ranges.set(m.id, { min: Math.min(...perPlayer), max: Math.max(...perPlayer) });
    // σ del grupo de puesto: lo que convierte el % en un z-score ("a cuántas desviaciones está").
    sds.set(m.id, sdOf(perPlayer));
  }
  // Metadatos del grupo para la UI (cuántos compañeros, a qué nivel se agrupó, qué medida se usó
  // y la dispersión). Van como propiedades del Map y no como claves, para no colisionar nunca
  // con un id de métrica.
  out.__peers = peers;
  out.__level = level;
  out.__stat = stat;
  out.__range = ranges;
...
// Desviación estándar MUESTRAL (n−1) del conjunto de referencia. Devuelve null con menos de
// MIN_SD valores: con dos datos la σ es ruido, y un z calculado sobre ruido se lee como si
// significara algo. Sin σ la card degrada a comparación por porcentaje, que sí es honesta.
export const MIN_SD = 3;
export function sdOf(vals) {
  const c = (vals || []).filter(v => v != null && !isNaN(v));
  if (c.length < MIN_SD) return null;
  const mu = c.reduce((a, b) => a + b, 0) / c.length;
  const varianza = c.reduce((a, b) => a + (b - mu) * (b - mu), 0) / (c.length - 1);
  const sd = Math.sqrt(varianza);
  // σ=0 (todos idénticos) haría el z infinito: se trata como "sin dispersión", igual que sin σ.
  return sd > 0 ? sd : null;
}

/**
 * Referencia PLANTEL: μ y σ de cada métrica sobre todos los jugadores del rango.
 *
 * Es la referencia que pide la matriz de z-score: cada celda es un jugador contra la
 * distribución de sus compañeros en esa métrica. A diferencia de fetchRoleBaseline, acá NO se
 * agrupa por puesto ni se excluye a nadie — el conjunto es el plantel entero, que es lo que
 * hace comparables las columnas entre sí.
 *
 * Los volúmenes pasan a media POR SESIÓN (roleNormAgg), igual que en el baseline de puesto: un
 * acumulado mezcla cuánto jugó cada uno con cómo rindió.
 *
 * @returns {Promise<Map<string, number|null>>} metricId → μ, con __sd (metricId → σ|null) y __n.
 */
export async function fetchSquadStats(sessionIds, config, ctx, catalog, sb) {
  if (!sessionIds.length) return new Map();
  const squadCfg = { ...config, scope: { level: 'squad' } };
  const rows = await fetchReports(sessionIds, squadCfg, ctx, catalog, sb);
  if (!rows.length) return new Map();
  ...
  for (const m of config.metrics) {
    ...
    for (const row of rows) {
      ...
      const b = byPlayer.get(row.player_id);
      b.vals.push(v);
      b.weights.push(_roleRowWeight(row));
    }
    const perPlayer = Array.from(byPlayer.values())
      .map(b => applyAgg(b.vals, roleNormAgg(m.agg), b.weights))
      .filter(v => v != null && !isNaN(v));
    n = Math.max(n, perPlayer.length);
    out.set(m.id, perPlayer.length ? applyAgg(perPlayer, 'avg') : null);
    sds.set(m.id, sdOf(perPlayer));
  }
  out.__sd = sds;
  out.__n  = n;
...
/**
 * Referencia "vs Sí mismo": μ y σ del PROPIO jugador sobre las sesiones del rango.
 *
 * Es la referencia que el selector ofrecía desde hacía tiempo sin que nada la implementara: la
 * card dibujaba los valores crudos y no avisaba. También es la base de la carta de control, que
 * pregunta exactamente esto — si una sesión se salió de lo normal PARA ÉL.
 *
 * A diferencia de la referencia de puesto o de plantel, acá el conjunto son las sesiones del
 * propio jugador, así que la σ mide cuánto se mueve él habitualmente. Los volúmenes NO se
 * reescalan por exposición: comparar a alguien consigo mismo no mezcla a nadie.
 *
 * @returns {Promise<Map<string, number|null>>} metricId → μ, con __sd (metricId → σ|null) y __n.
 */
export async function fetchSelfBaseline(sessionIds, config, ctx, catalog, sb) {
  if (!sessionIds.length) return new Map();
  const pid = config.scope?.playerId || ctx.playerId;
  if (!pid) return new Map();

  const cfg = { ...config, scope: { level: 'player', playerId: pid } };
  const rows = await fetchReports(sessionIds, cfg, ctx, catalog, sb);
  if (!rows.length) return new Map();
  ...
  for (const m of config.metrics) {
    const formula = isCalculated(m, catalog) ? formulaOf(m, catalog) : null;
    const cell = formula ? (row => evalFormulaRow(formula, b => cellVal(row, b))) : (row => cellVal(row, m.id));
    // Un valor por SESIÓN: si una sesión trae varias filas (períodos), se agregan primero, para
    // que una sesión partida en trozos no pese más que una entera en la media ni en la σ.
    const porSesion = new Map();
    for (const row of rows) {
      const v = cell(row);
      if (v == null || isNaN(v)) continue;
      const sid = row.session_id ?? row.training_sessions?.id ?? row.id;
      if (!porSesion.has(sid)) porSesion.set(sid, []);
      porSesion.get(sid).push(v);
    }
    const vals = Array.from(porSesion.values())
      .map(vs => applyAgg(vs, m.agg === 'total' ? 'total' : 'avg'))
      .filter(v => v != null && !isNaN(v));
    n = Math.max(n, vals.length);
    out.set(m.id, vals.length ? applyAgg(vals, 'avg') : null);
    sds.set(m.id, sdOf(vals));
  }
  out.__sd = sds;
  out.__n  = n;
  return out;
...
export async function fetchMdBaseline(sessionIds, config, ctx, catalog, sb) {
  ...
  const { data: curTs, error: e1 } = await sb.from('training_sessions')
    .select('id,session_attributes,match_day_offset').in('id', sessionIds);
  if (e1) throw new Error(`fetchMdBaseline: ${e1.message}`);
  const wantMd = new Set();
  for (const s of (curTs || [])) {
    const mc = mdRawOf(s);
    if (mc) wantMd.add(String(mc));
  }
  ...
  for (const row of rows) {
    const mc = mdRawOf(row.training_sessions);
    if (!mc) continue;
    const key = String(mc);
  ...
  for (const m of config.metrics) {
    ...
    // σ entre sesiones equivalentes: cuánto se mueve NORMALMENTE el jugador en ese MD. Es la
    // referencia de la card vieja "variación vs sesiones equivalentes".
    sds.set(m.id, sdOf(vals));
  }
  out.__sd = sds;
  return out;
...
export function enrichMcDiff(series, refSeries) {
  // Alineado por ÍNDICE: ambos sets salen del mismo pipeline (1:1 con config.metrics).
  // Keyear por label rompería con una métrica repetida (mismo id, distinto agg).
