...
export function aggregateSeries(rows, eavMap, config, catalog) {
  ...
  // Group key: explicit dimension if chosen, else legacy per-viz default.
  const dimId   = config.dimensions?.[0]?.id || null;
  const GROUPED = ['bars', 'ranking', 'table', 'heatmap', 'scatter', 'line'].includes(config.viz);
  const groupOf = dimId
    ? (row => dimGroup(row, dimId))
    : (config.viz === 'line'
        ? (row => { const t = timeLabel(row); return { key: t, label: t }; })
        : (row => ({ key: row.player_id, label: playerLabel(row) })));

  return config.metrics.map(m => {
    const cat = catalog.get(m.id);

    if (GROUPED) {
      // group rows by the dimension key → aggregate each metric within the group
      const byKey = new Map();
      for (const row of rows) {
        const g = groupOf(row);
        if (!byKey.has(g.key)) byKey.set(g.key, { label: g.label, vals: [] });
