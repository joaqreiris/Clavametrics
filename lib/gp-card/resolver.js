...
export function aggregateSeries(rows, eavMap, config, catalog) {
  const GROUP_BY_PLAYER = ['bars','ranking','table','heatmap','scatter'].includes(config.viz);
  const GROUP_BY_TIME   = config.viz === 'line';
  ...
  return config.metrics.map(m => {
    ...
    if (GROUP_BY_PLAYER) {
      // group rows by player_id → aggregate within group
      const byPlayer = new Map();
      for (const row of rows) {
        if (!byPlayer.has(row.player_id)) byPlayer.set(row.player_id, { label: playerLabel(row), vals: [] });
        const v = rowVal(row, m.id);
        if (v != null && !isNaN(v)) byPlayer.get(row.player_id).vals.push(v);
      }
      const points = Array.from(byPlayer.values())
        .map(({ label, vals }) => ({ x: label, y: applyAgg(vals, m.agg) ?? 0 }))
        .filter(p => p.y != null);
    ...
    if (GROUP_BY_TIME) {
      // group by session_date (or MD code)
      const byDate = new Map();
      for (const row of rows) {
        const lbl = timeLabel(row);
        if (!byDate.has(lbl)) byDate.set(lbl, []);
        const v = rowVal(row, m.id);
        if (v != null && !isNaN(v)) byDate.get(lbl).push(v);
      }
      const points = Array.from(byDate.entries())
        .map(([x, vals]) => ({ x, y: applyAgg(vals, m.agg) ?? 0 }));
