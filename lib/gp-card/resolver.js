...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  const _ck = `rpts:${ctx.clubId}:${ctx.teamId||''}:${_pid}:${(ctx.teamPlayerIds||[]).length}:${_coreK}:${sessionIds.slice().sort().join(',')}`;
  const _hit = _cacheGet(_ck);
  if (_hit !== undefined) return _hit;
  return _cacheSet(_ck, (async () => {
  ...
  let q = sb
    ...
    .select(select)
    .eq('club_id', ctx.clubId)
    .eq('is_invalid', false)          // exclude outlier-flagged rows from aggregates
    .in('session_id', sessionIds);

  if (config.scope.level === 'player') {
    const pid = config.scope.playerId || ctx.playerId;
    // Use the CHOSEN player as-is — never silently swap to another (that hid a
    // historical player's data, or showed always the same one). The sessions are
    // already team-scoped (getSessionIds), so the player only ever resolves within
    // this team's data; no rows in the range → empty, which is correct.
    ...
    // bound the rows to THIS team — including players who have since left — so adding
    // the current-roster filter here would hide exactly the historical data we want.
