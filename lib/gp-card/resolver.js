...
export async function getSessionIds(range, ctx, sb) {
  ...
  switch (range.type) {
    ...
    case 'season': {
      if (range.from) {                          // explicit season window
        q = q.gte('session_date', range.from);
        if (range.to) q = q.lte('session_date', range.to);
        break;
      }
      // No explicit season → derive it from the club/team's LATEST session, NOT from "today"
      // (today breaks on migrated/historical data, where the last session can be months old
      // and a 183-day window from today returns nothing). Season ≈ July 1 of that season.
      const { data: last } = await sb
      ...
      q = q.gte('session_date', `${startYear}-07-01`);
      break;
    }
    case 'custom': {
...
export function aggregateSeries(rows, eavMap, config, catalog) {
  ...
  function dimGroup(row, dimId) {
    ...
    switch (dimId) {
      ...
      // display = nombre del MC. Asociación por FECHA (session_date → ventana del MC) porque
      // training_sessions.microcycle_id suele venir null; se prefiere el id explícito si existe.
      case 'microcycle':   {
        const _sd = row.training_sessions?.session_date ?? row.session_date;
        const _dateId = (typeof window !== 'undefined' && window._gpMcForDate) ? window._gpMcForDate(_sd) : '';
        fid = row.training_sessions?.microcycle_id ?? (_dateId || null);
        const _lbl = (typeof window !== 'undefined' && window._gpMcLabelById) ? window._gpMcLabelById[fid] : null;
        v = attrs.microcycle || _lbl || fid;
        break;
      }
  ...
  return config.metrics.map(m => {
    ...
    if (GROUPED) {
      ...
      for (const row of rows) {
        const g = groupOf(row);
        if (!byKey.has(g.key)) byKey.set(g.key, { label: g.label, dims: g.dims, cat: g.cat, pid: g.pid, fid: g.fid, vals: [], weights: [] });
        const v = valOf(row);
        if (v != null && !isNaN(v)) { const e = byKey.get(g.key); e.vals.push(v); e.weights.push(rowWeight(row)); }
      }
      const points = Array.from(byKey.values())
        .map(({ label, dims, cat, pid, fid, vals, weights }) => ({ x: label, dims, cat, pid, fid, y: applyAgg(vals, m.agg, weights) ?? 0 }));
