...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  const _ck = `rpts:${ctx.clubId}:${_pid}:${ctx.teamId||''}:${_coreK}:${sessionIds.slice().sort().join(',')}`;
  const _hit = _cacheGet(_ck);
  if (_hit !== undefined) return _hit;
  return _cacheSet(_ck, (async () => {
  ...
  const select = [
    ...
    'players!inner(id, first_name, last_name, position, team_id)',
    ...coreNeeded,
  ].join(',');
  ...
  if (isUuid(ctx.teamId)) q = q.eq('players.team_id', ctx.teamId);
