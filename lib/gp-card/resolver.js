...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  const _ck = `rpts:${config.source||'session'}:${ctx.clubId}:${ctx.teamId||''}:${_pid}:${(ctx.teamPlayerIds||[]).length}:${sessionIds.slice().sort().join(',')}`;
  const _hit = _cacheGet(_ck);
  if (_hit !== undefined) return _hit;
  return _cacheSet(_ck, (async () => {
...
export function aggregateSeries(rows, eavMap, config, catalog) {
  ...
  return config.metrics.map(m => {
    ...
    if (GROUPED) {
      ...
      for (const row of rows) {
        const g = groupOf(row);
        if (!byKey.has(g.key)) byKey.set(g.key, { label: g.label, dims: g.dims, cat: g.cat, pid: g.pid, vals: [], weights: [] });
        const v = valOf(row);
        if (v != null && !isNaN(v)) { const e = byKey.get(g.key); e.vals.push(v); e.weights.push(rowWeight(row)); }
      }
      const points = Array.from(byKey.values())
        .map(({ label, dims, cat, pid, vals, weights }) => ({ x: label, dims, cat, pid, y: applyAgg(vals, m.agg, weights) ?? 0 }));
