...
export function canUsePlayerAgg(config, catalog) {
  ...
  const isKpi = viz === 'kpi';
  // GROUPED (tabla/barras/ranking por jugador) o KPI plantel (valor único, pooled o rollup 2-niveles).
  // line/heatmap/scatter/radar → crudo (agrupamiento temporal/otro no reconstruible desde por-jugador).
  if (!isKpi && !['table', 'bars', 'ranking'].includes(viz)) return false;
  const dimIds = (config.dimensions || []).map(d => d.id).filter(Boolean);
  if (isKpi) {
    if (dimIds.length) return false;                       // KPI con dimensión → no es el caso simple
  ...
  for (const m of ms) {
    ...
    if (isCalculated(m, catalog)) return false;           // métrica calculada (fórmula) → crudo
    if (DERIVED[m.id]) {                                  // acc_dec = accelerations+decelerations (suma de bases)
      if (!(m.agg === 'total' || m.agg === 'avg' || m.agg === 'wavg')) return false;  // max/min de la suma no reconstruible
      continue;                                           //   total/avg/wavg → desde columnas base
    }
...
// Σ de una métrica en el grupo (maneja DERIVADAS: acc_dec = accelerations_sum + decelerations_sum).
function _paSum(row, m) {
  const b = DERIVED[m.id];
  return b ? b.reduce((s, x) => s + (Number(row[`${x}_sum`]) || 0), 0) : (Number(row[`${m.id}_sum`]) || 0);
}
// Σ(valor·peso) de una métrica en el grupo (idem derivadas, para wavg).
function _paSwv(row, m) {
  const b = DERIVED[m.id];
  return b ? b.reduce((s, x) => s + (Number(row[`${x}_swv`]) || 0), 0) : (Number(row[`${m.id}_swv`]) || 0);
}

/** Valor de una métrica para un jugador, reconstruido de los bloques del RPC = applyAgg exacto.
 *  Soporta core y DERIVADAS (acc_dec) para total/avg/wavg. max/min de una derivada NO se reconstruye
 *  (max(a+d) ≠ max(a)+max(d)) → el guard excluye esos casos, así que aquí max/min sólo llega a core. */
function _paMetricValue(row, m) {
  ...
  switch (m.agg) {
    case 'total': return _paSum(row, m);                                      // Σ coalesce(x,0)
    case 'avg':   return _paSum(row, m) / n;                                  // Σ / count(*) (NULL cuenta como 0)
    case 'max':   { const v = row[`${m.id}_max`]; return v == null ? null : Number(v); }
    case 'min':   { const v = row[`${m.id}_min`]; return v == null ? null : Number(v); }
    case 'wavg':  { const sw = Number(row.w_sum) || 0; return sw > 0 ? _paSwv(row, m) / sw : null; }
    default:      return null;
  }
}
  ...
  return (config.metrics || []).map(m => {
    ...
    if (isKpi) {
      ...
      if (rollup) {
        ...
        const perPlayer = [];
        for (const row of rows) { const v = _paMetricValue(row, m); if (v != null && !isNaN(v)) perPlayer.push(v); }
        y = applyAgg(perPlayer, rollup) ?? 0;
      } else {
        y = _paPooled(rows, m) ?? 0;
      }
      return { label: m.id, name, unit, points: [{ x: 'all', y: (y == null || isNaN(y)) ? 0 : y }] };
    }
    const points = rows.map(row => {
      const label = _paPlayerLabel(row);
      const y = _paMetricValue(row, m);
...
export function canUsePlayerMcAgg(config, catalog) {
  ...
  for (const m of ms) {
    ...
    if (DERIVED[m.id]) {                                  // acc_dec: total/avg/wavg reconstruibles desde columnas base
