...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  try {
    if (typeof window !== 'undefined' && window.cmFetchAll) {
      ...
      const _S = _wc.length ? _wc : ['team'];   // contextos efectivos (default: solo team)
      ...
      const _plims = _pidScoped ? [_pid] : _rosterLimit;
      // Contextos no-team seleccionados: la etiqueta suele vivir en PERÍODOS, no en la fila de
      // sesión → sumar las filas sintetizadas desde gps_period_reports (ver _ctxPeriodRows).
      const _wcNT = _S.filter(c => c !== 'team');
      if (!isTask && _wcNT.length) {
        const extra = await _ctxPeriodRows(sb, ctx, sessionIds, _wcNT, _plims, rows, select);
        if (extra.length) rows.push(...extra);
      }
      // Modelo B: recorte dinámico — filas con trabajo de OTROS contextos se recalculan a los
      // períodos de los contextos pedidos, o se eliminan si no tienen ninguno (ver _applyCtxScope).
      if (!isTask && rows.length) {
        return await _applyCtxScope(sb, ctx, sessionIds, _S, _plims, rows);
      }
      return rows;
    }
