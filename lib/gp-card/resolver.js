...
export async function fetchRoleBaseline(sessionIds, config, ctx, catalog, sb) {
  ...
  // Metadatos del grupo para la UI (cuántos compañeros y a qué nivel se agrupó). Van como
  // propiedades del Map y no como claves, para no colisionar nunca con un id de métrica.
  out.__peers = peers;
  out.__level = level;
  return out;
}
