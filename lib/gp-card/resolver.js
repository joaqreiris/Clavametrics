 ...
 */
// ── Fase 2b: restar los PERÍODOS no-team del total de sesión ──────────────────────
// Un jugador que hace 60' de partido + top-up en la MISMA actividad tiene UNA fila en
// gps_reports (partido+top-up juntos). Restamos la suma de sus períodos no-team (rehab/
// individual/top-up) para que la media de equipo cuente solo el partido. Solo métricas de
// VOLUMEN (aditivas) — max/avg velocidad y m/min quedan como el valor de sesión (un top-up
// casi nunca toca el pico). El grano de la resta es (session_id, player_id).
const _ADDITIVE_COLS = ['total_distance','high_speed_distance','very_high_speed_distance','sprint_distance','sprint_count','accelerations','decelerations','player_load','hmld','time_played'];
// keepSet = contextos SELECCIONADOS en el filtro Context (además de 'team', que nunca se resta).
// Un período cuyo contexto está en keepSet NO se resta → su volumen se ve en el total. Default
// (keepSet vacío) = se restan todos los no-team, como antes.
async function _nonTeamPeriodSums(sb, clubId, sessionIds, playerIds, keepSet) {
  if (!sessionIds || !sessionIds.length) return new Map();
  if (typeof window === 'undefined' || !window.cmFetchAll) return new Map();
  const sel = ['session_id','player_id','work_context', ..._ADDITIVE_COLS].join(',');
  const rows = await window.cmFetchAll(() => {
    let q = sb.from('gps_period_reports').select(sel)
      .eq('club_id', clubId).neq('work_context', 'team').in('session_id', sessionIds);
    if (playerIds && playerIds.length) q = q.in('player_id', playerIds);
    return q;
  }, { label: 'resolver.nonTeamPeriods' });
  const m = new Map();
  for (const r of rows) {
    if (keepSet && keepSet.has(r.work_context)) continue;   // contexto seleccionado → NO se resta
    const k = r.session_id + '|' + r.player_id;
    let acc = m.get(k); if (!acc) { acc = {}; m.set(k, acc); }
    for (const c of _ADDITIVE_COLS) acc[c] = (acc[c] || 0) + (Number(r[c]) || 0);
  }
  return m;
}
function _subtractPeriods(rows, sums) {
  if (!sums.size) return rows;
  for (const r of rows) {
    const s = sums.get(r.session_id + '|' + r.player_id);
    if (!s) continue;
    for (const c of _ADDITIVE_COLS) {
      if (r[c] == null) continue;
      const adj = Number(r[c]) - (s[c] || 0);
      r[c] = adj > 0 ? adj : 0;   // sin negativos por ruido / períodos que sumen más que el total
    }
  }
  return rows;
}
...
export async function fetchReports(sessionIds, config, ctx, catalog, sb) {
  ...
  try {
    if (typeof window !== 'undefined' && window.cmFetchAll) {
      ...
      // Fase 2b: restar períodos no-team del total de sesión (solo gps_reports, no la task view).
      // La resta RESPETA el filtro Context: los contextos seleccionados (_wc) NO se restan, así
      // su volumen se ve en el total (ej. seleccionar los 4 → volumen completo).
      if (!isTask && rows.length) {
        const _keep = new Set(_wc);
        const sums = await _nonTeamPeriodSums(sb, ctx.clubId, sessionIds, _pidScoped ? [_pid] : (ctx.teamPlayerIds || null), _keep);
        _subtractPeriods(rows, sums);
      }
      return rows;
    }
