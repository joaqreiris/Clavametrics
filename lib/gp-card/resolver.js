...
export function canUsePlayerAgg(config, catalog) {
  ...
  for (const m of ms) {
    ...
    if (!_PLAYER_AGG_AGGS.has(m.agg)) return false;       // median/count → crudo (nivel 1 no reconstruible)
    ...
    if (DERIVED[m.id]) return false;                      // acc_dec (suma de bases) → crudo
...
function _paMetricValue(row, m) {
  ...
  const id = m.id;
  switch (m.agg) {
    case 'total': return Number(row[`${id}_sum`]) || 0;                       // Σ coalesce(x,0)
    case 'avg':   return (Number(row[`${id}_sum`]) || 0) / n;                 // Σ / count(*) (NULL cuenta como 0)
    case 'max':   { const v = row[`${id}_max`]; return v == null ? null : Number(v); }
    case 'min':   { const v = row[`${id}_min`]; return v == null ? null : Number(v); }
    case 'wavg':  { const sw = Number(row.w_sum) || 0; return sw > 0 ? (Number(row[`${id}_swv`]) || 0) / sw : null; }
    default:      return null;
  }
}
...
function _paPooled(rows, m) {
  ...
  for (const row of rows) {
    ...
    if (!rn) continue;
    any = true;
    sum += Number(row[`${id}_sum`]) || 0;
    n   += rn;
    sw  += Number(row.w_sum) || 0;
    swv += Number(row[`${id}_swv`]) || 0;
