...
export function canUsePlayerAgg(config, catalog) {
  ...
  if (config.scope?.rollup && config.scope.rollup !== 'pooled') return false;   // rollup 2-niveles → crudo
  if (config.comparison?.baseline && config.comparison.baseline !== 'none') return false;  // vs mc/role/match → crudo
  if (!['table', 'bars', 'ranking'].includes(config.viz)) return false;         // line/heatmap/scatter/radar/kpi → crudo
  const dimIds = (config.dimensions || []).map(d => d.id).filter(Boolean);
  if (dimIds.length && !(dimIds.length === 1 && dimIds[0] === 'player_name')) return false;  // sólo agrupado por jugador
  const ms = config.metrics || [];
  if (!ms.length) return false;
  for (const m of ms) {
    if (m.rel) return false;                              // Δ% vs MD/MC → crudo
    if (!_PLAYER_AGG_AGGS.has(m.agg)) return false;       // median/count → crudo
...
/** Construye la serie (misma forma que aggregateSeries GROUPED por jugador) desde las filas del RPC. */
