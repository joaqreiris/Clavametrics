...
(function () {
  ...
  window.cmResolveDayContext = async function (dateStr, teamId, clubId) {
    const out = { microcycle:null, md:'', start_time:null, duration:null, estimated_rpe:null, opponent:null, rival:null, home_away:null, hasEvent:false };
    ...
    try {
      ...
      let eq = window.sb.from('calendar_events')
        .select('type,start_time,duration,estimated_rpe,opponent,title,session_type,microcycle')
      ...
      if (ev) { out.hasEvent = true; out.start_time = ev.start_time||null; out.duration = ev.duration||null; out.estimated_rpe = ev.estimated_rpe||null; out.opponent = ev.opponent||out.rival; }
