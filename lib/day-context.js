...
(function () {
  ...
  window.cmResolveDayContext = async function (dateStr, teamId, clubId) {
    const out = { microcycle:null, md:'', start_time:null, duration:null, estimated_rpe:null, opponent:null, rival:null, home_away:null, hasEvent:false, event_id:null };
    ...
    try {
      ...
      let eq = window.sb.from('calendar_events')
        .select('id,type,start_time,duration_minutes,estimated_rpe,opponent,title,location')
      ...
      if (ev) { out.hasEvent = true; out.event_id = ev.id||null; out.start_time = ev.start_time||null; out.duration = ev.duration_minutes||null; out.estimated_rpe = ev.estimated_rpe||null; out.opponent = ev.opponent||out.rival; }
