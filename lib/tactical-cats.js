...
(function(){
  ...
  // acompañan. En los gráficos no pueden contar lo mismo: si contaran igual, dos
  // secundarios de relleno taparían al objetivo que de verdad sostuvo la sesión.
  // Un principal vale lo que TRES secundarios — el secundario sigue apareciendo
  // (se trabajó) pero no manda. Los KPIs y el gráfico semanal siguen mostrando
  // objetivos contados de a uno: el peso solo entra donde la pregunta es "qué
  // proporción de lo trabajado fue a esto", que es donde importa la jerarquía.
  // Fila sin `priority` (las cargadas antes de que existiera la división) =
  // principal, que es lo que eran: el objetivo del día.
  const W_MAIN = 1, W_SEC = 1 / 3;
  ...
  const api = {
    ...
    W_MAIN: W_MAIN,
    W_SEC: W_SEC,
    isMain(row){ return (row && row.priority) !== 'secondary'; },
    weight(row){ return api.isMain(row) ? W_MAIN : W_SEC; },
    // Orden de lectura de un día: los principales arriba, siempre.
    prioRank(row){ return api.isMain(row) ? 0 : 1; },
